...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    const diff1BigNumber = new BigNumber(global.diff1.toString());
    this.target = rpcData.b ? new BigNumber(rpcData.b) : new BigNumber(util.bignumFromBitsHex(rpcData.bits).toString());
    this.difficulty = diff1BigNumber.dividedBy(this.target).toNumber();
