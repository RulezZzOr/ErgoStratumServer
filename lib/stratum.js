...
var StratumClient = function(options){
    ...
    this.sendMiningJob = function(jobParams){
        ...
        try {
            var targetSource = params[6];
            var targetBn = new BigNumber(
                targetSource && typeof targetSource.toString === 'function'
                    ? targetSource.toString(10)
                    : String(targetSource)
            );

            var difficultyValue = this.difficulty && this.difficulty > 0 ? this.difficulty : 1;
            var difficultyBn = new BigNumber(difficultyValue);
            if (!difficultyBn.isFinite() || difficultyBn.lte(0)) {
                difficultyBn = new BigNumber(1);
            }

            var adjustedTarget = targetBn.dividedBy(difficultyBn);
            if (adjustedTarget.lte(0)) {
                adjustedTarget = new BigNumber(1);
            }

            params[6] = adjustedTarget.integerValue(BigNumber.ROUND_FLOOR).toString(10);
        } catch (err) {
            params[6] = String(jobParams[6]);
        }
        sendJson({
            id    : null,
