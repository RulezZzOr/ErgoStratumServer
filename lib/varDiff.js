...
class VarDiff extends events.EventEmitter {
  constructor(opts = {}) {
    ...
    if (typeof opts.targetTime === 'number') {
      merged.targetTime = opts.targetTime;
    }

    if (typeof opts.retargetTime === 'number') {
      merged.retargetInterval = opts.retargetTime;
      if (typeof merged.retargetShares !== 'number' || !Number.isFinite(merged.retargetShares) || merged.retargetShares <= 0) {
        const shares = Math.max(1, Math.round(opts.retargetTime / (merged.targetTime || 1)));
        merged.retargetShares = shares;
      }
      if (typeof merged.emaHalflife !== 'number' || !Number.isFinite(merged.emaHalflife) || merged.emaHalflife <= 0) {
        merged.emaHalflife = opts.retargetTime / 2;
      }
    }

    if (typeof opts.variancePercent === 'number') {
      const variance = Math.max(0, opts.variancePercent) / 100;
      const up = 1 + variance;
      const down = Math.max(0.1, 1 - variance);
      merged.maxStepUp = Math.max(up, merged.maxStepUp);
      merged.maxStepDown = Math.min(down, merged.maxStepDown);
    }
