...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    **/
    ...
    this.setVarDiff = function(port, varDiffConfig) {
        ...
        var derivedOptions = Object.assign({}, varDiffConfig || {});
        var baseDiff = options.ports[port] && options.ports[port].diff;
        if (typeof baseDiff === 'number' && isFinite(baseDiff)) {
            if (typeof derivedOptions.startDiff === 'undefined')
                derivedOptions.startDiff = baseDiff;
            if (typeof derivedOptions.minDiff === 'undefined')
                derivedOptions.minDiff = Math.max(1, baseDiff / 4);
            if (typeof derivedOptions.maxDiff === 'undefined')
                derivedOptions.maxDiff = baseDiff * 16;
        }
        var varDiffInstance = new varDiff(derivedOptions);
        varDiffInstance.port = port;
        _this.varDiff[port] = varDiffInstance;
        _this.varDiff[port].on('newDifficulty', function(client, newDiff) {
            // Apply VarDiff immediately

            /* We request to set the newDiff @ the next difficulty retarget
             (which should happen when a new job comes in - AKA BLOCK) */
            client.enqueueNextDifficulty(newDiff);
            try { const job = _this.jobManager.currentJob.getJobParams(); client.sendMiningJob(job); } catch(e) {}
